		...
		<div class="viewlist">
			...
			<div class="flex topbar">
				...
				<div class="handlerbtn">
					<a-button class="ant-btn-success" v-has="'foun:school:add'" @click="addSchool">添加</a-button>
					<a-button type="danger" v-has="'foun:school:batchRemove'" @click="showDeleteConfirm(0)" :disabled="selectedRowKeys.length <= 0">删除</a-button>
				</div>
			</div>
			<!--表格-->
			<s-table class="mt20" bordered :columns="columns" :data="loadData" rowKey="id" ref="table" :scroll="{ x: 1300 }"
			 size="small" :rowSelection="{selectedRowKeys: selectedRowKeys, onChange: onSelectChange}">
				...
				<span slot="operation" slot-scope="text,record" class="operation">
					<a href="javascript:;" title="编辑" class="edit" v-has="'foun:school:edit'" @click="editSchool(record)"><i class="material-icons icon iconfont">border_color</i></a>
					<a href="javascript:;" title="删除" class="del" v-has="'foun:school:batchRemove'" @click="showDeleteConfirm(1,record.id)"><i
						 class="material-icons icon iconfont">delete</i></a>
				</span>
			</s-table>
		</div>
		<school-modal ref="schoolModal" @ok="handlerRefresh"></school-modal>
	</sub-page-view>
</template>

<script>
	import subPageView from '@/layouts/subPageView';
	import SchoolModal from './module/schoolModal'
	import {
		rowSelection
	} from '@/mixins'
	...
	export default {
		...
		methods: {
			...
			showDeleteConfirm(type, delId) {
				...
				this.$confirm({
					...
					onOk() {
						//console.log('OK');
						if (type == 1) {
							$me.removeSchool(delId);
						} else {
							$me.batchRemoveSchool();
						}
					},
					onCancel() {
						console.log('Cancel');
			...
			/* 添加 */
			addSchool() {
				this.$refs.schoolModal.add();
				//this.visible=true;
			},
			/* 编辑 */
			editSchool(record) {
				this.$refs.schoolModal.edit(record);
			},
			/*删除*/
			removeSchool(delId) {
				postAction(
					infoManageapi.removeSchool, {
						id: delId
					}).then(res => {
					this.handlerRefresh();
				});
			...
			batchRemoveSchool() {
				postAction(
					infoManageapi.batchRemoveSchool, this.selectedRowKeys).then(res => {
