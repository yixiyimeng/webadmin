	...
	export default {
		...
		methods: {
			search() {
				...
				// 	if (!filterparam[key]) {
				// 		this.$message.error('请选择校区，班级和主题');
				// 		return false;
				// 	}
				// }
			...
			getToplist(paramer) {
				if (paramer.classCode) {
					postAction(api.getTopiclistbyDate, paramer).then(da => {
			...
			getSchoollist(startDatetime, ) {
				...
				this.schoollist = [];
				if (startDatetime) {
					postAction(api.getSchoollistbyDate, {
			...
			getClasslist(paramer) {
				...
				if (paramer.schoolCode) {
					postAction(api.getClasslistbyDate, paramer).then(da => {
						if (da.code == 0) {
			...
			getTopicDate(paramer) {
				...
				/* 查询有主题的日期 */
				postAction(api.getTopicDate, param).then(da => {
					/* 设置有效日期 */
					console.log(da.data);
