		...
		<a-spin :spinning='confirmLoading'>
			<a-form :form="form" autocomplete="off">
				...
				<a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="类型">
					<a-select @change="onChangeType" v-decorator="[ 'type', {rules: [{ required: true, message: '科目' } ],initialValue:'common' }]">
						<a-select-option value="common">普通试卷</a-select-option>
						<a-select-option value="classify">套题试卷</a-select-option>
					</a-select>
				</a-form-item>
				<a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="上传">
					<div class="flex flex-pack-justify flex-align-center">
						...
						<div>
						<a :href="fileUrl" class="ant-btn ant-btn-primary" download="普通试卷模板.xlsx">
							<a-icon type="download" /><span>普通模板下载</span></a>
						<a :href="fileUrl2" class="ant-btn ant-btn-primary ml20" download="套题试卷模板.xlsx">
							<a-icon type="download" /><span>套题模板下载</span></a>
							</div>
					</div>
				</a-form-item>
...
<script>
	...
	export default {
		...
		created() {
			...
			this.fileUrl2 = baseURL + '/plat/files/test-template-classify.xlsx'
		},
		methods: {
			add() {
				this.title = '新增试卷';
			...
			handleUpload() {
				...
				this.form.validateFields((err, values) => { // 验证表单没错误
					if (!err) {
						...
						postActionUpload(_this.isAdd ? infoManageapi.addTestPaper : infoManageapi.updateTestPaper, formData).then(res => {
							if (res.code == 0) {
			...
			onChangeType(value) {
				...
				/* 切换类型，更换模板地址 */
				// if (value == 'classify') {
				// 	this.fileUrl = this.baseURL + '/plat/files/test-template_classify.xlsx'
				// }
			}
