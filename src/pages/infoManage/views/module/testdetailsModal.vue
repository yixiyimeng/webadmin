	...
	export default {
		...
		methods: {
			...
			loadData() {
				//做一些异步操作
				postAction(
					infoManageapi.getAllTestQuestionList, {
			...
			handleOK() {
				/* 批量插入或者修改 */
				let paramer = pick(this.record, 'titleCode', 'titleName', 'classCode', 'className', 'subjectCode', 'subjectName');
				if (this.list && this.list.length > 0) {
					paramer.questions = this.list.map((item, index) => {
						var obj = pick(item, 'questionType', 'score', 'trueAnswer', 'questionId', 'id');
						obj.questionId =this.listmap&&this.listmap[item.maxNum]&&this.listmap[item.maxNum].length>1?item.num:item.maxNum
						return obj
					})
				}
				postAction(
					infoManageapi.testbatchInsertOrUpdate, paramer
			...
			showDeleteConfirm(record, index) {
				...
				this.$confirm({
					...
					onOk() {
						//console.log('OK');
						if (record.id) {
							$me.removequestion(record.id, index);
						} else {
