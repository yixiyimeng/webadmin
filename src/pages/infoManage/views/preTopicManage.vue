<template>
	<sub-page-view>
		...
		<!-- 添加，编辑 -->
		<topic-manage-modal ref="topicManageModal" @ok="handlerRefresh"></topic-manage-modal>
		<topicdetails-modal ref="testdetailsModal"></topicdetails-modal>
	</sub-page-view>
</template>

<script>
	import subPageView from '@/layouts/subPageView';
	import TopicManageModal from './module/topicManageModal';
	import topicdetailsModal from './module/topicdetailsModal';
	import {
		infoManageapi,
		getAllSubjectList
	} from '../api';
	...
	export default {
		...
		created() {
			...
			getAllSubjectList().then(da => {
				...
				if(list&&list.length>0){
					...
					this.superQueryFieldList[2].optionlist.unshift({value:-1,text:'全部'})
				}
		...
		methods: {
			...
			/* 添加 */
			addTopicQuestion() {
				this.$refs.topicManageModal.add();
				//this.visible=true;
			},
			/* 编辑 */
			editTopicQuestion(record) {
				this.$refs.topicManageModal.edit(record);
			},
			/*删除*/
			removeTopic(delId) {
				postAction(
					infoManageapi.removeTopic, {
						id: delId
					}).then(res => {
					this.handlerRefresh();
				});
			...
			batchRemoveTopic() {
				postAction(
					infoManageapi.batchRemoveTopic, this.selectedRowKeys).then(res => {
